...
async def insert_db(original_url: str, shorten_url: str, session: db.SessionDep) -> None:
    db_url = db_models.Urls(original_url=original_url, shorten_uri=shorten_url)
    session.add(db_url)
    session.commit()
...
async def short_uri_exists(short_uri: str, session: db.SessionDep) -> bool:
    url_by_shorten = session.exec(
            sqlmodel.select(db_models.Urls).where(db_models.Urls.shorten_uri == short_uri)
        )
...
@URLS_ROUTER.post(
        ...
        responses={
            201: {
                ...
                'model': models.ShortenUriResponse,
                'content': {
                    'application/json': {
...
async def insert_url(url_body: models.ShortenUriRequest, session: db.SessionDep) -> models.ShortenUriResponse:
    ...
    if row:
        shorten_url = DOMAIN + '/api/v1/url/' + row.shorten_uri
        return models.ShortenUriResponse(shorten_uri=shorten_url)
    
    if url_body.shorten_uri:
        uri_exists = await short_uri_exists(url_body.shorten_uri, session)
        if uri_exists:
            raise exceptions.ShortenUriAlreadyExists(short_url=url_body.shorten_uri)
        
        shorten_url = DOMAIN + '/api/v1/url/' + url_body.shorten_uri
        await insert_db(url_body.original_url, url_body.shorten_uri, session)
    ...
    while True:
        ...
        attempt += 1
        if attempt == 20:
            raise exceptions.ShortenUriAlreadyExists(shorten_uri)
...
async def get_original_url(shorten_uri: str, session: db.SessionDep):
    url_by_shorten = session.exec(
            sqlmodel.select(db_models.Urls).where(db_models.Urls.shorten_uri == shorten_uri)
        )
    row = url_by_shorten.one_or_none()
    if not row:
        raise exceptions.ShortenUriDoesNotExist(shorten_uri)
